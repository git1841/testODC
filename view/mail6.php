...
foreach ($clients as $client) {
    ...
    foreach ($paiements as $paiement) {
        ...
        // Compteurs liés
        $queryCompteur = $pdo->prepare("SELECT codecompteur, pu FROM COMPTEUR WHERE codecli = ?");
        $queryCompteur->execute([$codecli]);
        $compteurs = $queryCompteur->fetchAll(PDO::FETCH_ASSOC);
        ...
        foreach ($compteurs as $compteur) {
            ...
            // ---------------- ÉLECTRICITÉ ----------------
            $queryElec = $pdo->prepare("SELECT date_presentation, date_limite_paie, valeur1 FROM ELEC WHERE codecompteur = ?");
            ...
            if ($elecData) {
                ...
                $valeur1 = $elecData['valeur1'] ?? 0;
                $resteElec = $montant - ($pu * $valeur1);

                $datePaie = new DateTime($datepaie);

                $etatElec = ($datePaie >= $datePresentation && $datePaie <= $dateLimitePaie)
            ...
            // ---------------- EAU ----------------
            $queryEau = $pdo->prepare("SELECT date_releve2, date_presentation2, date_limite_paie2, valeur2 FROM EAU WHERE codecompteur = ?");
            ...
            if ($eauData) {
                ...
                $valeur2 = $eauData['valeur2'] ?? 0;
                $resteEau = $montant - ($pu * $valeur2);

                $datePaieEau = new DateTime($datepaie);

                $etatEau = ($datePaieEau >= $datePresentationEau && $datePaieEau <= $dateLimitePaieEau)
